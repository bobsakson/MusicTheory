...
function Quiz() {
    ...
    //   shouldForwardProp: (prop) => prop !== 'invalidNote',
    // })(({ invalidNote }) => ({
    //   ...(invalidNote &&
    //     {
    //       backgroundColor: red,
    ...
    const checkAnswers = () => {
      // noteRefs.current.forEach((value, index) => {
      //   noteRefs.current[index].invalidNote = false;
      // });
      //elRef.current.style.backgroundColor = "red";
      answerKey.forEach((value, index) => {
        if(value !== answers[index]) {
          // noteRefs.current[index].sx = {backgroundColor: red};
    ...
    const setSelectedAnswer = (e, note) => {
      ...
      //const noteIndex = newAnswers.indexOf(note);
    ...
    const populateKeyNotes = (selectedKey) => {
        ...
        const ak = majorKeys.find(x => x.majorKey === selectedKey).notes.filter(note => note !== selectedKey);
